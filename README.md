...
Maintainer: Xiaoai. Release: 2026-09-19. Target: `final draft0918.docx`, with the author-confirmed Table 5, Sobel, and binary-income Full Chow specifications.

This repository reconstructs new `charls.dta` and `dataset 0507.dta` files from the supplied upstream sources, runs the retained manuscript models, and produces 19 numbered aggregate tables and six figures. No previous 0507, previous CHARLS panel, or saved individual analysis sample is an input.

## Quick start

1. Obtain the licensed CHARLS raw modules, Harmonized CHARLS C/D files, PSU files, and the external city/COVID/PM2.5 source files listed in `verification/INPUT_MANIFEST.json`. These are **not included**. The municipal input must be the archived **2011–2020 workbook that includes all four municipalities**.
...
The final success message is `REPRODUCTION_PACKAGE_RUN_VERIFIED`. Also check that the new run contains `verification_passed.ok` and that `verification.json` has `passed: true`. Stata's batch-process exit code by itself is not sufficient.

Optional data-only reconstruction:

```stata
do RUN_ALL.do build
```

Each invocation makes a fresh directory under `runs/`. The build never falls back to an old dataset if an input is missing. Harmonized files are upstream inputs; the code does not claim to reconstruct every Harmonized variable from raw questionnaires.

## Directory structure

```text
RUN_ALL.do                 Entry point: new raw build, models, figures, export, validation
...
- A1a: traditional Sobel with city-clustered path SEs and the original path-specific samples, plus a joint-covariance delta-method diagnostic. Both are unadjusted supplementary diagnostics.
...
- Heterogeneity: nine descriptive subgroup models, but only the five approved pooled Group × DID tests in A3. The middle-income descriptive subgroup remains in Table 4; there is no additional middle-income A3 interaction.
...
- Nonlinear fits: full controls and explicit city/year indicators with city clustering. Ordered and binary logit are **diagnostics only**, with separation/rank warnings retained. The previous nonconverged generalized ordered logit is not a retained robustness result.
- Respondent fixed effects: year effects and city clustering; singleton exclusions and absorbed controls are documented.
- Income missingness: wave/treatment rates and retained-versus-missing comparisons only. No new imputation, observation weighting, or enlarged-sample DID has been added.
...
The supplied draft0918 Table 1 still displays COVID Obs 86,696 and SD 0.228. The final data have **86,654 nonmissing COVID rows** and SD **0.2297963855**, reported as **0.230**. The 42 missing rows are environmental-only records. This package exports the correct values; it does not alter data to match stale manuscript cells. The principal sample remains 16,661 observations and 7,995 respondents.
