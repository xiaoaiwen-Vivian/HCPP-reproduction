* Xiaoai: A1a Sobel formula and joint city-covariance diagnostic.
version 18.0
clear all
set more off
set type double
...
assert e(N)==16661
tempname h
postfile `h' str10 Mediator str16 Sample double(a se_a b se_b N_a N_b G_a G_b Indirect Sobel_SE Sobel_z Sobel_p Joint_cov Joint_SE Joint_z Joint_p Joint_se_a Joint_se_b) using "`output'/sobel_city.dta", replace
foreach item in "Green GreenCoverageRateBD" "Social act_12" {
 tokenize `"`item'"'
 local lab "`1'"
 local med "`2'"
 foreach spec in path_specific {
  local cond "`eligible'"
  ...
  assert sx_na==16968 if "`lab'"=="Green" & "`spec'"=="path_specific"
  assert sx_na==16876 if "`lab'"=="Social" & "`spec'"=="path_specific"
